...
describe('highlightPayload', () => {
  it('uses the typed grammar map without interpreting source as markup', () => {
    ...
    expect(highlightPayload('<main>literal</main>', 'html-file')).toContain('hljs-tag');
    expect(highlightPayload('@@ -1 +1 @@\n-old\n+new', 'diff')).toContain('hljs-meta');
  });

  it('returns null for unknown languages and payloads over the shared size guard', () => {
