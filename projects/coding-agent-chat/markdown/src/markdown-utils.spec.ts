...
describe('highlightPayload', () => {
  it('maps typed payloads through the shared registered grammars', () => {
    ...
    expect(highlightPayload('<main>safe</main>', 'html-file')?.join('\n')).toContain('hljs-tag');
  });
  ...
  it('escapes source markup in sanitizer-safe output', () => {
    const highlighted = highlightPayload('<script>alert(1)</script>', 'html-file')?.join('\n');
