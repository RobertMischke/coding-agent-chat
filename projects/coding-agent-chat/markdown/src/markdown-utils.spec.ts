...
describe('highlightPayload', () => {
  it('uses the registered grammar for each typed payload kind', () => {
    ...
    const html = highlightPayload('<html><body>safe</body></html>', 'html-file');
  ...
  it('falls back for unknown code grammars and oversized payloads', () => {
    expect(highlightPayload('plain readable text', 'code-block', 'unknown-language')).toBeNull();
