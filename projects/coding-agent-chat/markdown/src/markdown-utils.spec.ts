...
describe('highlightPayload', () => {
  it('uses the typed diff, JSON, and HTML grammars', () => {
    ...
    expect(highlightPayload('<main>ready</main>', 'html-file')).toContain('hljs-tag');
  });

  it('uses a code-block language hint and safely falls back for unsupported input', () => {
    expect(highlightPayload('public sealed class Worker {}', 'code-block', 'csharp')).toContain(
      'hljs-keyword',
    );
