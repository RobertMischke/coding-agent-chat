...
describe('highlightPayload', () => {
  it('maps typed payloads to the shared grammars and escapes source HTML', () => {
    expect(highlightPayload('public sealed class Job {}', 'code-block', 'csharp')).toContain(
      'hljs-keyword',
    );
    const diff = highlightPayload('+added\n-removed\n@@ -1 +1 @@', 'diff');
    expect(diff).toContain('hljs-addition');
    expect(diff).toContain('hljs-deletion');
    expect(diff).toContain('<span class="hljs-meta">@@ -1 +1 @@</span>');
    expect(highlightPayload('{"ready": true}', 'json')).toContain('hljs-attr');

    const html = highlightPayload('<main data-state="ready">hello</main>', 'html-file');
    expect(html).toContain('hljs-tag');
    expect(html).not.toContain('<main');
  });

  it('returns null for unknown code grammars and oversized payloads', () => {
