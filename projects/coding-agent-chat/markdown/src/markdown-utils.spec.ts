...
describe('highlightPayload', () => {
  it('uses typed grammars and returns sanitizer-safe, balanced lines', () => {
    const diff = highlightPayload(
      'diff --git a/a.txt b/a.txt\n--- a/a.txt\n+++ b/a.txt\n@@ -1 +1 @@\n-old\n+new',
      'diff',
    );
    const html = highlightPayload('<main data-id="fixture">ok</main>', 'html-file');
  ...
  it('returns null for unknown grammars and inputs over the shared size guard', () => {
    expect(highlightPayload('plain readable source', 'code-block', 'unknown-grammar')).toBeNull();
