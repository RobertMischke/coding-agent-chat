...
describe('highlightPayload', () => {
  ...
  it('returns the plain-text signal for unknown grammars and oversized payloads', () => {
    expect(highlightPayload('plain', 'code-block', 'unknown-language')).toBeNull();
    expect(highlightPayload('x'.repeat(60_001), 'json')).toBeNull();
  });

  it('escapes source markup in its sanitizer-safe token fragments', () => {
    const highlighted = highlightPayload('<script>alert("x")</script>', 'html-file')?.join('\n');
