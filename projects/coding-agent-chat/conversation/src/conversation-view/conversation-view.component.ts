...
export class ConversationViewComponent {
  ...
  /**
   * Return sanitizer-safe highlight spans for typed source payloads. The
   * shared Markdown helper owns grammar registration, caching, and fallback
   * limits; this component only joins its balanced per-line output.
   */
  highlightedPayloadHtml(payload: MessageContentPayload): string | null {
    switch (payload.type) {
      case 'code-block':
        return highlightPayload(payload.text, payload.type, payload.language)?.join('\n') ?? null;
      case 'diff':
      case 'json':
      case 'html-file':
        return highlightPayload(payload.text, payload.type)?.join('\n') ?? null;
      default:
        return null;
    }
  }
