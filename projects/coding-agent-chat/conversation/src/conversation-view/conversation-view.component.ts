...
export class ConversationViewComponent {
  ...
  /** Highlight typed source payloads without routing raw file content through Markdown. */
  highlightedPayload(payload: MessageContentPayload): HighlightedPayload | null {
    switch (payload.type) {
      case 'code-block':
        return highlightPayload(payload.text, payload.type, payload.language ?? null);
      case 'diff':
      case 'json':
      case 'html-file':
