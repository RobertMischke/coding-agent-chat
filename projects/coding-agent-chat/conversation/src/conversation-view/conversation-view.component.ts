...
export class ConversationViewComponent {
  ...
  /**
   * Highlight typed non-Markdown payloads without teaching this renderer
   * about grammars, caches, or tokenizer limits owned by the Markdown entry
   * point. A null result deliberately preserves the plain-text fallback.
   */
  protected highlightedPayload(payload: MessageContentPayload): readonly string[] | null {
    switch (payload.type) {
      case 'code-block':
