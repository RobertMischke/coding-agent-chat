...
export class ConversationViewComponent {
  ...
  /**
   * Render typed source payloads through the Markdown package's shared
   * highlighter. A null result deliberately leaves the template on its plain
   * text path for raw logs, unknown grammars, and oversized payloads.
   */
  renderHighlightedPayload(payload: MessageContentPayload): string | null {
    switch (payload.type) {
      case 'code-block':
