...
import { highlightPayloadLines, MarkdownViewComponent } from 'coding-agent-chat/markdown';
...
type RenderRow =
  ...
  | { kind: 'unknownFrame'; id: string; event: SystemUnknownFrameEvent }
...
export class ConversationViewComponent {
  ...
  /**
   * Route typed source payloads through the Markdown entry point's shared
   * highlighter. A null result deliberately keeps the template on its escaped
   * text fallback for unknown grammars and payloads above the size guard.
   */
  highlightedPayload(payload: MessageContentPayload): readonly string[] | null {
    switch (payload.type) {
      case 'code-block':
        return highlightPayloadLines(payload.text, payload.type, payload.language);
      case 'diff':
      case 'json':
      case 'html-file':
        return highlightPayloadLines(payload.text, payload.type);
      default:
        return null;
    }
  }
    ...
    for (const e of this.events()) {
      ...
      switch (e.kind) {
        ...
        case 'system.status':
          ...
          break;
        case 'system.unknownFrame':
          out.push({ kind: 'unknownFrame', id: e.id, event: e });
          break;
