...
describe('ConversationViewComponent', () => {
  ...
  it('syntax-highlights typed code and renders git payloads as visual diffs', async () => {
    ...
    const html = '<main data-ready="true">safe</main>';
    const fixture = await render([
    ...
    expect(renderedHtml?.querySelector('.hljs-tag')?.textContent).toContain('<main');
    expect(renderedHtml?.querySelector('main')).toBeNull();
  });
