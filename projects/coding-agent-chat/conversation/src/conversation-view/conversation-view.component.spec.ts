...
describe('ConversationViewComponent', () => {
  ...
  it('syntax-highlights typed source payloads and renders line-level visual diff classes', async () => {
    // Exact git diff shape captured in codex/0.146.x/content-matrix.stream.json.
    const diff = [
  ...
  it('keeps unknown typed code readable through the plain-text fallback', async () => {
    const source = '<widget>**literal**</widget>';
    const fixture = await render([
      msg('message.taskAgent', source, {
        content: [{ type: 'code-block', text: source, language: 'unknown-language' }],
      }),
    ]);
    ...
    expect(payload?.textContent).toBe(source);
    expect(payload?.querySelector('widget')).toBeNull();
  });
