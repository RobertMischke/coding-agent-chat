...
describe('ConversationViewComponent', () => {
  ...
  it('renders typed diffs with line-level classes from the captured Codex fixture', async () => {
    const diff = [
  ...
  it('highlights typed C#, JSON, and HTML payloads with their mapped grammars', async () => {
    ...
    expect(host.querySelector('[data-payload-type="code-block"] .hljs-keyword')).toBeTruthy();
  ...
  it('keeps unknown and oversized typed code payloads on the plain-text fallback', async () => {
    ...
    const fixture = await render([
      msg('message.taskAgent', 'plain unknown source', {
        content: [
          { type: 'code-block', text: 'plain unknown source', language: 'unknown-language' },
        ],
      }),
    ...
    expect(payloads[0].textContent).toBe('plain unknown source');
