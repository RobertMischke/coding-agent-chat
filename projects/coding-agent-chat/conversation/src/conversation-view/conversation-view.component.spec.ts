...
describe('ConversationViewComponent', () => {
  ...
  it('keeps unknown, oversized, and raw-log payloads on the plain-text path', async () => {
    const oversized = 'x'.repeat(60_001);
    const log = '[INFO] plain log';
    const fixture = await render([
      msg('message.taskAgent', 'readable', {
        content: [{ type: 'code-block', text: 'readable', language: 'unknown-language' }],
      }),
    ...
    expect(payloads[0].textContent).toBe('readable');
    expect(payloads[1].classList).not.toContain('md-code--hl');
    expect(payloads[1].textContent).toBe(oversized);
    expect(payloads[2].getAttribute('data-payload-type')).toBe('raw-log');
    expect(payloads[2].classList).not.toContain('md-code--hl');
