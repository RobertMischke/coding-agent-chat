...
describe('ConversationViewComponent', () => {
  ...
  it('syntax-highlights typed source payloads and renders visual diff line classes', async () => {
    const diff = [
    ...
    const fixture = await render([
      msg('message.taskAgent', 'Typed payload matrix', {
        content: [
          ...
          { type: 'code-block', text: 'public class Foo {}', language: 'csharp' },
  ...
  it('keeps unknown and oversized typed code payloads on the readable plain-text path', async () => {
    const oversized = 'x'.repeat(60_001);
    const fixture = await render([
      msg('message.taskAgent', 'Fallback payloads', {
        content: [
          { type: 'code-block', text: '<unknown source>', language: 'unknown-grammar' },
    ...
    expect(payloads[0].textContent).toBe('<unknown source>');
