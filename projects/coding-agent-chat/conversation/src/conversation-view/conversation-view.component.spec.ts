...
describe('ConversationViewComponent', () => {
  ...
  it('renders the captured Codex git diff with class-tagged additions, deletions, and hunks', async () => {
    const diff = [
  ...
  it('falls back to plain readable source for unknown grammars and oversized payloads', async () => {
    const unknown = '<not-markup>wat source</not-markup>';
    ...
    const fixture = await render([
      ...
      msg('message.taskAgent', oversized, {
        content: [{ type: 'json', text: oversized }],
      }),
    ]);
    const host = fixture.nativeElement as HTMLElement;
    const unknownPayload = host.querySelector<HTMLElement>('[data-payload-type="code-block"]');
    const oversizedPayload = host.querySelector<HTMLElement>('[data-payload-type="json"]');

    expect(unknownPayload?.classList).not.toContain('md-code--hl');
    expect(unknownPayload?.querySelector('[class*="hljs-"]')).toBeNull();
    expect(unknownPayload?.querySelector('not-markup')).toBeNull();
