...
describe('ConversationViewComponent', () => {
  ...
  it('syntax-highlights typed code and renders line-level visual diffs', async () => {
    const diff = [
    ...
    const csharp = ['public sealed class Foo', '{', '    public int X { get; set; }', '}'].join(
      '\n',
    );
    const fixture = await render([
      ...
      msg('message.taskAgent', '<main data-state="ready">Hello</main>', {
        content: [
          {
            type: 'html-file',
            text: '<main data-state="ready">Hello</main>',
            mediaType: 'text/html',
          },
        ],
      }),
    ...
    expect(diffPayload?.querySelector('.hljs-meta')?.textContent).toBe('@@ -1 +1 @@');
    ...
    expect(htmlPayload?.textContent).toBe('<main data-state="ready">Hello</main>');
