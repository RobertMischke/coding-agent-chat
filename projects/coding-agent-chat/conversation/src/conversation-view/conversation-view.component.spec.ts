...
describe('ConversationViewComponent', () => {
  ...
  it('renders typed diff lines with visual addition, deletion and hunk classes', async () => {
    const diff = [
  ...
  it('syntax-highlights typed C#, JSON and HTML payloads without parsing raw files', async () => {
    const csharp = 'public class Foo { public int X { get; set; } }';
    const json = '{"ok":true}';
    const html = '<!doctype html><html><body><ul><li>Item</li></ul></body></html>';
    const fixture = await render([
  ...
  it('keeps unknown and oversized typed code payloads as readable plain text', async () => {
    ...
    const fixture = await render([
      ...
      msg('message.taskAgent', oversized, {
        content: [{ type: 'code-block', text: oversized, language: 'json' }],
      }),
    ]);
    ...
    expect(payloads[0].querySelector('widget')).toBeNull();
