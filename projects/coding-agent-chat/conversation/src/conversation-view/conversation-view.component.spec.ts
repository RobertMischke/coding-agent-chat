...
describe('ConversationViewComponent', () => {
  ...
  it('keeps unknown and oversized code payloads on the plain-text path', async () => {
    const unknown = '<not-markup>readable</not-markup>';
    const oversized = `public class Large { /* ${'x'.repeat(60_001)} */ }`;
    const fixture = await render([
    ...
    expect(payloads[0].querySelector('not-markup')).toBeNull();
