...
describe('ConversationViewComponent', () => {
  ...
  it('syntax-highlights typed code, JSON, and HTML payloads with the shared grammars', async () => {
  ...
  it('renders typed git diffs with class-tagged additions, deletions, and hunk headers', async () => {
    const diff = [
  ...
  it('keeps unknown and oversized typed code payloads on the plain-text fallback', async () => {
    const unknown = '<unsafe-tag>plain</unsafe-tag>';
    ...
    expect(payloads[0].querySelector('unsafe-tag')).toBeNull();
