...
export function projectConversation(ctx: ConversationProjectionContext): ConversationEvent[] {
  ...
  for (let i = 0; i < groups.length; i++) {
    ...
    // Protocol drift is higher-signal than the generic no-action rule. Only
    // recognised runtime frames may disappear into a phase/Trace; an unknown
    // future frame must still reach projectGroup() and become the typed
    // system.unknownFrame warning introduced by the versioned protocol corpus.
    const hasProtocolDrift =
      group.runtimeFrame !== undefined &&
      (group.sourceLines ?? group.lines).some(
        (line) => inspectCliFrame(line.text, ctx.frameSource ?? null)?.known === false,
      );
    const material = hasProtocolDrift ? null : classifyWorkMaterial(group);
    if (material) {
