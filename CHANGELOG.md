...
- Typed `code-block`, `diff`, `json`, and `html-file` payloads now reuse the
  Markdown renderer's syntax highlighter in the conversation view, including
  line-level addition, deletion, and hunk-header styling for git diffs.
